...
This runs the native chunky-to-planar golden vectors and three-layer graphics oracle, compiles and inspects the Hunk executables, runs the hosted bootstrap through `vamos` and FS-UAE, then executes the 256 × 256 AGA screen regression under FS-UAE.
...
See the [macOS development toolchain guide](documentation/macos-development-toolchain.md) for local ROM/HDF configuration and validated versions, the [three-layer graphics reference](documentation/graphics-reference-compositor.md) for canonical composition semantics, the [C2P reference note](documentation/c2p-reference.md) for the existing plane-byte contract, the [layout benchmark](documentation/c2p-layout-benchmark.md) for the open representation decision, and the [AGA screen smoke-test note](documentation/aga-screen-smoke.md) for the provisional playfield mapping and current validation boundary.
