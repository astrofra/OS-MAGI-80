...
assignments, comparisons, nested `if`/`else`, and nested `while` is parsed into
a bounded AST, lowered to a typed multi-block stack IR, and interpreted as a host oracle. ABI
...
resolves parallel edge copies including cycles, preserves used saved registers,
...
4. **Semantic expansion — Phase 3 (`bool`, comparisons, `if`/`else`, and `while` implemented):** the typed IR now has bounded cyclic multi-block control flow, loop-carried O1 value joins, fixed-point CFG liveness, `phi`-slot coalescing, and parallel edge-copy resolution. Add fixed point, globals, arrays, records, dictionaries, strings/symbols, guards, `break`/`continue`, runtime mocks, negative tests, and deterministic randomized differential tests.
...
6. **Performance regression — Phase 3 onward (cyclic CFG allocation signals implemented):** the `-O0`/`-O1` differential records code size, executed instructions, and maximum callee stack use for six reviewed corpora, including typed locals, a nested conditional CFG, and a loop with a cyclic parallel copy, plus a forced spill fixture. The conditional case verifies CFG-aware register reuse and the coalescing of six live `phi` values into two stack slots; the loop case reduces 316/315/48 code-bytes/instructions/stack-bytes at O0 to 196/166/28 at O1. Approximate core cycles, calls, memory-operation counts, and representative cartridge kernels remain pending. Never translate emulator values into A1200 frame claims.
