...
3. **ABI freeze — early Phase 3 (register/stack tranche implemented):** ABI 0.1 now fixes register roles, opaque `A5` runtime-context ownership, scalar returns, Boolean representation, frame bounds, stack alignment, and executable saved-register checks. Calls, locals, multiple returns, context layout, and error traps remain open extensions.
...
6. **Performance regression — Phase 3 onward (initial size/count signals implemented):** the `-O0`/`-O1` differential now records code size and executed instructions for three reviewed straight-line corpora. Approximate core cycles, stack depth, calls, memory operations, and representative cartridge kernels remain pending. Never translate emulator values into A1200 frame claims.
