...
3. **ABI freeze — early Phase 3 (register/stack and spill-frame tranches implemented):** ABI 0.1 now fixes register roles, opaque `A5` runtime-context ownership, scalar returns, Boolean representation, frame bounds, stack alignment, and executable saved-register checks. The `-O1` allocator emits bounded `A6` spill frames and restores `D3-D7/A6`; calls, source locals, multiple returns, context layout, and error traps remain open extensions.
...
6. **Performance regression — Phase 3 onward (initial size/count/stack signals implemented):** the `-O0`/`-O1` differential now records code size, executed instructions, and maximum callee stack use for three reviewed straight-line corpora and a forced spill fixture. Approximate core cycles, calls, memory-operation counts, and representative cartridge kernels remain pending. Never translate emulator values into A1200 frame claims.
