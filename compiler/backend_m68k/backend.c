...
{
    static const char *const argument_registers[MIGA80_MAX_PARAMETERS] = {
        "%d0", "%d1", "%d2"
    };
    unsigned int index;
    ...
    if (!output_line(output,
                     "/* Generated by miga80c; development oracle only. */\n"
                     "        .text\n"
                     "        .even\n"
                     "        .globl  %s\n"
                     "%s:\n"
    ...
    for (index = 0; index < function->parameter_count; ++index) {
        if (!output_line(output, "        move.l  %s,-%u(%%a6)\n",
                         argument_registers[index], (index + 1U) * 4U)) {
            return output_failure(diagnostic, NULL);
        }
    }
