...
static enum Magi80C2P4Status validate_m68k_conversion(
...
static enum Magi80C2P4Status validate_mask32_m68k_conversion(
    const uint8_t *chunky,
    size_t width,
    size_t height,
    size_t chunky_stride,
...
#if defined(__m68k__)
extern void magi80_c2p4_pair_lut_m68k_packed4_core(
    const uint8_t *chunky,
...
extern void magi80_c2p4_pair_lut_m68k_byte4_core(
    const uint8_t *chunky,
    size_t groups_per_row,
    size_t height,
    size_t source_row_skip,
...
extern void magi80_c2p4_mask32_m68k_packed4_core(
    const uint8_t *chunky,
...
extern void magi80_c2p4_mask32_m68k_byte4_core(
    const uint8_t *chunky,
    size_t blocks_per_row,
    size_t height,
    size_t source_row_skip,
...
enum Magi80C2P4Status magi80_c2p4_mask32_m68k_packed4(
    ...
    size_t plane_stride)
{
    enum Magi80C2P4Status status = validate_mask32_m68k_conversion(
...
enum Magi80C2P4Status magi80_c2p4_pair_lut_m68k_packed4(
    ...
    const uint32_t table[MAGI80_C2P4_PAIR_LUT_ENTRIES])
{
    enum Magi80C2P4Status status = validate_m68k_conversion(
        chunky, width, height, chunky_stride, width >> 1, planes,
        plane_stride, table);
...
#if defined(__m68k__)
    magi80_c2p4_pair_lut_m68k_packed4_core(
...
#if defined(__m68k__)
    magi80_c2p4_pair_lut_m68k_byte4_core(
